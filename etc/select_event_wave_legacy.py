...
FILE_ROOT_DIR = '/media/jjong/로컬 디스크2/data/VITALDB/' ## vital sign 경로
SAVE_ROOT_DIR = '/mnt/storage1/datathon2019/extracted/' 
#FILE_ROOT_DIR = '/Volumes/Fast SSD/sign/'
#SAVE_ROOT_DIR = '/Volumes/Fast SSD/event_wave/'
VITAL_TRKS = pd.read_csv('/home/jjong/jjong/workspace/datathon_2019/data/VITAL_TRKS.csv') ## vital track 경로
VITAL_TRKS = VITAL_TRKS[VITAL_TRKS['gain']>0]
CASE_IDS = []
with open ('/home/jjong/jjong/workspace/datathon_2019/data/case_id_mbp.list','r',encoding='utf-8') as f:
...
def get_wave(event, nonevents,c_id,id_trks):          
    f_waves = id_trks['tname'][(id_trks['type']=='W')] ## wave file list read
    for f in f_waves: #['SNUADC/ART']: #
        ...
        try: 
            ...
            for chk_p in CHK_POINT: ## event case 저장
                ## event time 발생 prev_point*60초전 시간 get
                e_bound = ((event['first_dt'] - (chk_p*60)), (event['first_dt'] - (chk_p*60) +60))     
                wave_in_bound = df_wav[[f,'minute','second']][(df_wav['second']>=e_bound[0]) & (df_wav['second']<=e_bound[1])].reset_index(drop=True)              
                if f == 'SNUADC/ART':
                    wave_in_bound[f] = wave_in_bound[f]-512 ## 보정

                ## file 이름: caseid_wavename_minute
                if len(wave_in_bound)>100:
                    wave_in_bound.to_csv(os.path.join(SAVE_ROOT_DIR,"{:05d}_{}.{}M.e.csv".format(c_id,wav_f_name,chk_p)))
                ...
                if chk_p == 0:
                    ...
                    if f == 'SNUADC/ART':
                        wave_surround_in_bound[f] = wave_surround_in_bound[f]-512                
            ...
            for i in range(len(nonevents)): ## non-event case 저장
                n_bound = ((nonevents[i]['second']), (nonevents[i]['second'] + 60))
                df_wav[[f,'second']][(df_wav['second']>n_bound[0]) & (df_wav['second']<n_bound[1])].reset_index(drop=True).to_csv(os.path.join(SAVE_ROOT_DIR,"{:05d}_{}.n.{:03d}.csv".format(c_id,wav_f_name,i)))
...
#     progress_rate = 10   
    for i, case_id in enumerate (case_list,1):#list(false_2['case_id']): ## list 조정
#         current_rate = int((i/total_progress)*100)+1        
#         if current_rate %  progress_rate == 0:
#             print('{:d}% completed'.format(progress_rate))
#             progress_rate+=10

        case_id = int(case_id)
        print(case_id)
        case_info = VITAL_TRKS[(VITAL_TRKS['caseid']==case_id)]
        event_surround, event, nonevents = event_selection(case_id,case_info)

        if event: 
            pd.DataFrame(event[0])[['mbps','second']].to_csv(os.path.join(SAVE_ROOT_DIR,"{:05d}_{}.0M.e.csv".format(case_id,'MBP')))
            pd.DataFrame(event_surround[0])[['mbps','second']].to_csv(os.path.join(SAVE_ROOT_DIR,"{:05d}_{}.0M.e_surr.csv".format(case_id,'MBP')))
            
            first_event = surround_mbps(event[0]['event_idx'] ,event)
            first_surround = surround_mbps(event[0]['event_idx'] , event_surround)
            
            #event_wave, event_surround_wave = get_wave(event[0], nonevents,case_id,case_info) # for visualization
            get_wave(event[0], nonevents,case_id,case_info)
